...
SEARCH_EMPLOYEE_INFO_TOOL_DESCRIPTION="従業員情報CSVから従業員を検索する時に使う"
...
SYSTEM_PROMPT_EMPLOYEE_SEARCH = """
あなたは従業員情報データベースを参照し、ユーザーの質問に対して最適な従業員情報を検索・回答するAIアシスタントです。

【条件】
1. ユーザーの入力内容と従業員情報の中から関連性が高い情報を抽出してください。
2. 氏名、部署、役職、連絡先など、質問内容に応じて適切な項目を回答してください。
3. 関連する従業員が複数いる場合は、一覧で回答してください。
4. 関連する情報が見つからない場合は、「該当する従業員情報は見つかりませんでした。」と回答してください。
5. 回答は分かりやすく、必要に応じてマークダウン形式で出力してください。
...
SYSTEM_PROMPT_NOTICE_SLACK = """
    ...
    - 【メンション先の選定理由】の選定理由を、以下の条件に従って生成してください。
        - 誰を選定したのか。
        - なぜ、その人を選定したのか。
        - 複数人いるのであれば、上記を人数分記載してください。
